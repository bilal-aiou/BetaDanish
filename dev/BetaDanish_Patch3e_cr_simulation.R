## =============================================================================
##  BetaDanish  --  PATCH 3e : COMPETING-RISKS COVARIATES AND SIMULATION
## =============================================================================
##
##  Implements approved recommendations 37, 38 and 39, so that 0.3.0 ships
##  complete rather than deferring anything to 0.4.0.
##
##    37  covariates in fit_bd_competing(), entering each cause through an
##        accelerated failure time link, plus ED cause-specific kernels via
##        submodel = TRUE and a covariate-aware cif_betadanish()
##    38  bd_simulation_study()      univariate MLE, the G1 and G4 designs
##    39  bd_simulation_cure()       the G2 design
##        bd_simulation_competing()  the G3 and G5 designs
##        simulate_bd_competing_data()
##
##  HOW COVARIATES ENTER
##    Following thesis section 7.4 and .cr_cov_nll_ed() in the master script,
##    each cause carries its own coefficient vector acting on the time scale:
##
##        T_j = T_0j * exp(x'gamma_j)
##
##    so a positive coefficient lengthens time to failure from that cause. An
##    event at t contributes the log-density evaluated at t*exp(-x'gamma_j)
##    plus the Jacobian -x'gamma_j. The same design matrix serves every cause
##    but the coefficients are free to differ, so one covariate may accelerate
##    one cause and retard another.
##
##    The intercept is dropped from the design matrix: each cause already has
##    its own scale k, which an intercept would be confounded with.
##
##  cif_betadanish() GAINS A COVARIATE ARGUMENT
##    Scaling time by lambda maps k to k/lambda, so a covariate effect folds
##    exactly into the scale parameter. The CIF is evaluated at x = 0 by
##    default, the reference subject.
##
##  WHAT THE RUNNERS REPORT
##    Bias, RMSE, the ratio of mean estimated SE to empirical SD, and Wald
##    coverage. Read the last two together: a ratio well below one means the
##    asymptotic standard errors are optimistic, and coverage falls short as a
##    consequence. Failed fits are counted, not dropped -- a high rate is a
##    finding about the parameter region, not noise to be hidden.
##
##  HOW TO RUN   source("dev/BetaDanish_Patch3e_cr_simulation.R")
...
cat(strrep("=", 78), "\n")
cat("  BetaDanish  --  Patch 3e : competing-risks covariates and simulation\n")
cat(strrep("=", 78), "\n")
...
if (!any(grepl(".bd_make_accept", readLines("R/utils-internal.R", warn = FALSE), fixed = TRUE)))
  .die("Patch 2c has not been applied.")
.ok("Patches 1 through 3b detected")

BACKUP_DIR <- file.path(".betadanish_backup",
                        format(Sys.time(), "%Y%m%d-%H%M%S-patch3e"))
dir.create(BACKUP_DIR, recursive = TRUE, showWarnings = FALSE)
.ok(paste("backup:", BACKUP_DIR))

.step("Rec 37: rewriting R/competing_risks.R with covariate support")
...
#' \donttest{
#' set.seed(1)
#' d <- simulate_bd_competing_data(300, seed = 1)
#'
#' # No covariates
#' fit <- fit_bd_competing(d$time, d$cause, submodel = TRUE, n_starts = 3)
#' fit
#'
#' # One binary covariate, acting separately on each cause
#' fit2 <- fit_bd_competing(d$time, d$cause, covariates = ~ x, data = d,
...
#' @examples
#' \donttest{
#' d <- simulate_bd_competing_data(300, seed = 2)
#' fit <- fit_bd_competing(d$time, d$cause, submodel = TRUE, n_starts = 3)
...
.step("Rec 38, 39: writing R/simulation_study.R")
...
#' @examples
#' \donttest{
#' s <- bd_simulation_study(n = c(75, 150), n_sim = 40,
#'                          truth = c(b = 3, c = 2, k = 0.5),
#'                          submodel = TRUE, seed = 1)
#' s
#' }
...
#' @examples
#' \donttest{
#' s <- bd_simulation_cure(n = c(150), n_sim = 20, seed = 1)
#' s
#' }
...
#' @examples
#' \donttest{
#' s <- bd_simulation_competing(n = 200, n_sim = 15, seed = 1)
#' s
#' }
...
if (!any(grepl("bd_simulation_study", .nw, fixed = TRUE))) {
  .hdr <- grep("^# BetaDanish 0\\.2\\.0\\.9000|^# BetaDanish 0\\.3\\.0", .nw)
  if (length(.hdr) >= 1L) {
    .backup("NEWS.md")
    .sec <- c(
      "",
...
set.seed(101)
.d <- simulate_bd_competing_data(400, seed = 101)
.pass("simulated data has both causes and censoring",
      all(c(0, 1, 2) %in% unique(.d$cause)),
      paste("counts:", paste(table(.d$cause), collapse = "/")))

.f <- suppressWarnings(fit_bd_competing(.d$time, .d$cause, submodel = TRUE,
                                        n_starts = 2))
.pass("competing-risks fit returns positive parameters",
      all(.f$coefficients > 0) && is.finite(.f$logLik),
      sprintf("logLik = %.3f", .f$logLik))

## The CIFs of all causes cannot sum above one.
.cf <- cif_betadanish(.f, 50, 1) + cif_betadanish(.f, 50, 2)
.pass("cause CIFs sum to at most one", is.finite(.cf) && .cf <= 1 + 1e-6,
      sprintf("sum = %.5f", .cf))

## Folding a covariate into the scale must reproduce a direct refit of k.
.p0 <- BetaDanish:::.bd_cr_pars(.f, 1)
.pass("parameter extraction matches the coefficient matrix",
      isTRUE(all.equal(.p0$b, unname(.f$coefficients[1, "b"]))))

## The summary helper must reproduce textbook bias and RMSE.
set.seed(7); .e <- rnorm(400, 2.2, 0.5)
.s <- BetaDanish:::.bd_sim_summary(.e, rep(0.5, 400), truth = 2)
.pass("simulation summary reproduces bias and RMSE",
      isTRUE(all.equal(unname(.s["bias"]), mean(.e) - 2)) &&
        isTRUE(all.equal(unname(.s["rmse"]), sqrt(mean((.e - 2)^2)))))
...
for (f in c("simulate_bd_competing_data", "bd_simulation_study",
...
cat("\n", strrep("=", 78), "\n", sep = "")
cat("  PATCH 3e COMPLETE\n")
cat(strrep("=", 78), "\n\n")
cat("  37  covariates in fit_bd_competing(), ED kernels, covariate-aware CIF\n")
cat("  38  bd_simulation_study()\n")
cat("  39  bd_simulation_cure(), bd_simulation_competing(),\n")
cat("      simulate_bd_competing_data()\n\n")
cat("  Next: Patch 3f -- visualisation (recs 40-42) and the remaining\n")
cat("  vignettes (rec 46). Then re-run Patch 3d to release 0.3.0.\n\n")
