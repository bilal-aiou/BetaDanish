## =============================================================================
##  BetaDanish  --  PATCH 3a-fix2 : one documentation topic for the entropies
## =============================================================================
##
##  Patch 3a-fix moved the warning rather than removing it:
##
##    Duplicated \argument entries in Rd file 'bd_entropy_shannon.Rd':
##      'b' 'c' 'k' 'rel.tol' 'a' 'subdivisions'
##
##  Those six names are exactly the arguments bd_entropy_shannon() shares with
##  bd_moment_summary(), so two roxygen blocks were landing in one Rd topic. I
##  have now guessed at the mechanism twice and been wrong twice, so this patch
##  removes the possibility instead of diagnosing it.
##
##  bd_entropy_shannon(), bd_entropy_renyi() and bd_entropy_tsallis() now share
##  a single explicit topic, `@name bd_entropy`, with one @param tag per
##  argument across the union of their formals. A single block cannot contain
##  duplicate entries, whatever roxygen decides to merge.
##
##  The patch also scans EVERY generated Rd file for duplicated argument names
##  before running check(), so this class of fault is caught in seconds rather
##  than after a four-minute build, and prints the arguments block of the topic
##  that was failing.
##
##  ALSO
##    bd_entropy_renyi() and bd_entropy_tsallis() gain a `subdivisions`
##    argument, wired through to stats::integrate() alongside `rel.tol`.
##
##  Nothing about the mathematics changes.
##
##  HOW TO RUN   source("dev/BetaDanish_Patch3a_fix2.R")
##  IDEMPOTENT   Yes -- the file is written whole.
## =============================================================================
...
cat(strrep("=", 78), "\n")
cat("  BetaDanish  --  Patch 3a-fix2 : unified entropy topic\n")
cat(strrep("=", 78), "\n")
...
BACKUP_DIR <- file.path(".betadanish_backup",
                        format(Sys.time(), "%Y%m%d-%H%M%S-patch3afix2"))
dir.create(BACKUP_DIR, recursive = TRUE, showWarnings = FALSE)
.ok(paste("backup:", BACKUP_DIR))

.step("Clearing Rd files for the entropy topics so nothing stale survives")
for (f in c("man/bd_entropy_shannon.Rd", "man/bd_entropy_renyi.Rd",
            "man/bd_entropy_tsallis.Rd")) {
  if (file.exists(f)) { .backup(f); file.remove(f); .ok(paste("removed", f)) }
}

.step("Writing R/structural.R with a single entropy topic")
...
## ---------------------------------------------------------------------------
##  Scan every Rd file for duplicated argument names. This is the check that
##  R CMD check performs, run here in a second rather than after a four-minute
##  build.
## ---------------------------------------------------------------------------
.step("Scanning every man/*.Rd for duplicated argument entries")
...
.dupfiles <- character(0)
for (f in .rds) {
  nms <- .rd_arg_names(f)
  dup <- unique(nms[duplicated(nms)])
  if (length(dup)) {
    .warn(sprintf("%s duplicates: %s", basename(f), paste(dup, collapse = ", ")))
    .dupfiles <- c(.dupfiles, basename(f))
  }
}
if (length(.dupfiles)) {
  .die("Duplicated argument entries remain in: ",
       paste(.dupfiles, collapse = ", "),
       "\nR CMD check would report the same warning. Backups: ", BACKUP_DIR)
}
.ok(sprintf("%d Rd file(s) scanned, no duplicated arguments", length(.rds)))

.step("Showing the arguments of the unified entropy topic")
.ent <- file.path("man", "bd_entropy.Rd")
if (file.exists(.ent)) {
  nms <- .rd_arg_names(.ent)
  cat("        bd_entropy.Rd documents: ", paste(nms, collapse = ", "), "\n", sep = "")
  .ok(sprintf("%d argument(s), all distinct", length(nms)))
} else {
  .warn("man/bd_entropy.Rd was not generated; check the @name tag")
}

.step("Confirming the entropy functions are still exported")
.ns <- readLines("NAMESPACE", warn = FALSE)
for (f in c("bd_entropy_shannon", "bd_entropy_renyi", "bd_entropy_tsallis")) {
...
cat("\n", strrep("=", 78), "\n", sep = "")
cat("  PATCH 3a-fix2 COMPLETE\n")
cat(strrep("=", 78), "\n\n")
cat("  The three entropy functions share one topic, bd_entropy, with a single\n")
cat("  @param tag per argument. Renyi and Tsallis gained subdivisions.\n")
cat("  Every Rd file is now scanned for duplicated arguments before check().\n\n")
cat("  Expected: 0 errors, 0 warnings, 0-1 notes -- and Patch 3a closes.\n\n")
