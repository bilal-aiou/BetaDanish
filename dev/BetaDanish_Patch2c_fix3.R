## =============================================================================
##  BetaDanish  --  PATCH 2c-fix3 : normalise the diagnostics before testing
## =============================================================================
##
##  This is the third correction to the same twenty-line function, so it stops
##  being a spot fix. The whole class of fault is removed instead.
##
##  WHAT KEPT HAPPENING
##    fix2 replaced every guard with isTRUE(), which absorbs NA and zero-length
##    results. But in
##
##        isTRUE(abs(d$ac_correlation) > 0.95)
##
##    abs() runs FIRST. abs(NULL) is not numeric(0); it is an error. So a
##    diagnostics list without that field still failed, just with a different
##    message. Guarding conditions one at a time keeps leaving a gap.
##
##  WHAT THIS DOES INSTEAD
##    .bd_warn_diagnostics() now normalises every field to a length-one value
##    of the expected type before evaluating any condition. Anything absent,
##    empty, mis-shaped or non-numeric becomes NA_real_ or FALSE up front, so
##    no condition can ever see a value it cannot handle. The tests cover NULL
##    fields, zero-length fields, character fields, length-two fields, an
##    empty list and a non-list.
##
##  UNCHANGED AND WORKING
##    The degeneracy guard itself. The transplant fit stays at logLik -476.82
##    with chi-squared 0, against +5.2e77 before Patch 2c.
##
##  HOW TO RUN   source("dev/BetaDanish_Patch2c_fix3.R")
...
cat(strrep("=", 78), "\n")
cat("  BetaDanish  --  Patch 2c-fix3 : normalised diagnostics\n")
cat(strrep("=", 78), "\n")
...
BACKUP_DIR <- file.path(".betadanish_backup",
                        format(Sys.time(), "%Y%m%d-%H%M%S-patch2cfix3"))
...
.step("Exercising the warner against every awkward shape before check()")
.shapes <- list(
...
for (nm in names(.shapes)) {
  r <- tryCatch({
    suppressWarnings(BetaDanish:::.bd_warn_diagnostics(.shapes[[nm]])); "ok"
  }, error = function(e) conditionMessage(e))
  if (identical(r, "ok")) .ok(nm) else { .warn(paste0(nm, ": ", r)); .fail <- c(.fail, nm) }
}
if (length(.fail))
  .die("The warner still errors on: ", paste(.fail, collapse = ", "),
       "\n\nBackups: ", BACKUP_DIR)
.ok("no shape produces an error")
...
cat("\n", strrep("=", 78), "\n", sep = "")
cat("  PATCH 2c-fix3 COMPLETE\n")
cat(strrep("=", 78), "\n\n")
cat("  Diagnostics fields are normalised before any condition is evaluated,\n")
cat("  so no absent, empty, mis-shaped or non-numeric field can raise an\n")
cat("  error. Nine awkward shapes were exercised above, before check() ran.\n\n")
